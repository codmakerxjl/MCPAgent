...
def list_modules(
    offset: Annotated[int, "Offset to start listing from (start at 0)"],
    count: Annotated[int, "Number of modules to list (100 is a good default, 0 means remainder)"],
) -> Page[dict]:
    """List all modules in the database (paginated)"""
    modules = []
    mod = ida_idd.modinfo_t()
    t = ida_dbg.get_first_module(mod)
    while t:
        modules.append({
            "name": mod.name,
            "base": hex(mod.base),
            "size": hex(mod.size),
            "rebase_to": hex(mod.rebase_to),
        })
        t = ida_dbg.get_next_module(mod)
    return paginate(modules, offset, count)
...
def get_first_module() -> dict:
    ...
    t = ida_dbg.get_first_module(mod)
    if not t:
        raise IDAError("No module found")
    return {
        "name": mod.name,
        "base": hex(mod.base),
        "size": hex(mod.size),
...
@jsonrpc
@idaread
def get_module_info_by_address(
    address: Annotated[str, "Address to get the module info for"],
) -> dict:
...
def _get_module_info_by_name(
    name: str,
) -> Optional[ida_idd.modinfo_t]:
...
@jsonrpc
@idaread
def get_module_info_by_name(
    name: Annotated[str, "Name of the module to get the info for"],
) -> dict:
    """Get the module info for a given name"""
    mod = []
    for module in idautils.Modules():
...
@jsonrpc
@idaread
def get_next_module_by_name(
    name: Annotated[str, "Name of the module to get the next module for"],
) -> dict:
    """Get the next module in the database"""
    mod_iter = idautils.Modules()
    found = False
    for mod in mod_iter:
        if found:
            return {
...
@idawrite
def add_bpt(
    address: Annotated[str, "Address to add the breakpoint at"],
) -> None:
...
@idawrite
def del_bpt(
    address: Annotated[str, "Address to delete the breakpoint at"],
) -> None:
...
@idawrite
def enable_bpt(
    address: Annotated[str, "Address to enable the breakpoint at"],
) -> None:
    """Enable the breakpoint at the given address"""
    address = parse_address(address)
    ida_dbg.enable_bpt(address)

@jsonrpc
@idawrite
def disable_bpt(
    address: Annotated[str, "Address to disable the breakpoint at"],
) -> None:
...
@idaread
def exist_bpt(
    address: Annotated[str, "Address to check if the breakpoint exists at"],
) -> bool:
    """Check if a breakpoint exists at the given address"""
    address = parse_address(address)
    return ida_dbg.exist_bpt(address)
...
@idaread
def read_dbg_memory(
    address: Annotated[str, "Address to read from"],
    size: Annotated[int, "Size to read in bytes"],
) -> str:
    """Read memory from the debugger"""
    address = parse_address(address)
    data = ida_dbg.read_memory(address, size)
    return data.hex(" ")

@jsonrpc
@idaread
def export_functions_to_file(
    filename: Annotated[str, "Path to the file to export to"],
    offset: Annotated[int, "Offset to start listing from (start at 0)"],
    count: Annotated[int, "Number of functions to list (100 is a good default, 0 means remainder)"],
):
    """Export functions to a file"""
    functions = [get_function(address) for address in idautils.Functions()]
    functions = paginate(functions, offset, count)["data"]
    with open(filename, "w") as f:
        for function in functions:
            f.write(f"{function['address']}: {function['name']}\n")

@jsonrpc
@idaread
def get_prefix_functions(
    prefix: Annotated[str, "Prefix to filter functions by"],
    offset: Annotated[int, "Offset to start listing from (start at 0)"],
    count: Annotated[int, "Number of functions to list (100 is a good default, 0 means remainder)"],
) -> Page[Function]:
    """Get functions with a specific prefix (paginated)"""
    functions = [get_function(address) for address in idautils.Functions()]
    functions = pattern_filter(functions, prefix, "name")
    return paginate(functions, offset, count)
...
@jsonrpc
@idaread
def get_prefix_functions_count(
    prefix: Annotated[str, "Prefix to filter functions by"],
) -> int:
...
@jsonrpc
@idaread
def export_prefix_functions_to_file(
    filename: Annotated[str, "Path to the file to export to"],
    prefix: Annotated[str, "Prefix to filter functions by"],
    offset: Annotated[int, "Offset to start listing from (start at 0)"],
    count: Annotated[int, "Number of functions to list (100 is a good default, 0 means remainder)"],
...
@jsonrpc
@idawrite
def mark_fuction_green(
    address: Annotated[str, "Address of the function to mark"],
):
    ...
    idaapi.set_item_color(func.start_ea, 0x00FF00)  # Green color


@jsonrpc
@idawrite
def mark_function_red(
    address: Annotated[str, "Address of the function to mark"],
):
    ...
    if not func:
        raise IDAError(f"No function found at address {address}")
    idaapi.set_item_color(func.start_ea,0xFF0000)
